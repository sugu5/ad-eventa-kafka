...
start_metrics_server(port=8005)
logger.info("Prometheus metrics server started on port 8005")

from pyspark.sql import SparkSession
from pyspark.sql.functions import col, from_json, udf, to_timestamp, max as spark_max
...
# PostgresSQL Config - Update these with your local PostgreSQL credentials
...
def get_avro_schema_from_registry(schema_registry_url, subject):
    ...
    try:
        ...
        logger.warning(f"Schema Registry request failed, falling back to local schema: {e}")
        return None
    ...
    logger.warning(
        f"Schema Registry returned status {response.status_code} for subject {subject}, falling back to local schema"
    )
    return None
...
def get_kafka_end_offsets(topic_partition_pairs):
    """Fetch current broker end offsets for topic/partition pairs."""
    consumer = None
    try:
        ...
        topic_partitions = [TopicPartition(topic, partition) for topic, partition in topic_partition_pairs]
        ...
        logger.warning(f"Failed to fetch Kafka end offsets for lag metrics: {e}")
        return {}
    finally:
...
def process_batch(batch_df, epoch_id):
    ...
    # Update offset/lag gauges from Kafka metadata columns available on the raw stream.
    partition_rows = (batch_df
    ...
    for row in partition_rows:
        ...
        processed_offset = int(row["max_offset"])
        consumed_offsets[(topic, partition)] = processed_offset
        consumer_offset.labels(topic=topic, partition=str(partition)).set(float(processed_offset))

    end_offsets = get_kafka_end_offsets(list(consumed_offsets.keys()))
    for (topic, partition), processed_offset in consumed_offsets.items():
        end_offset = end_offsets.get((topic, partition))
        if end_offset is None:
            continue
        lag_value = max(int(end_offset) - (processed_offset + 1), 0)
        consumer_lag.labels(topic=topic, partition=str(partition)).set(float(lag_value))
